...
public class FrontControllerServlet extends HttpServlet {
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ControllerMethods methodeDeURL = urlToMethods.get(url);
        if (methodeDeURL == null) {
            String prefix = url.substring(0, url.lastIndexOf('/'));
            if (prefix.isEmpty()) prefix = "/";

            Map<String, ControllerMethods> matchingRoutes = new HashMap<>();
            for (Map.Entry<String, ControllerMethods> entry : urlToMethods.entrySet()) {
        ...
        request.setAttribute("controllerName", methodeDeURL.getControllerName());
        request.setAttribute("methodName", methodeDeURL.getMethode().getName());
        request.getRequestDispatcher("/route.jsp").forward(request, response);
    }
